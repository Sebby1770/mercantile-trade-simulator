...
const world=createWorld(scene),audio=createAudio(settings);
...
function pause(){paused=true;controls.disable();if(document.pointerLockElement)document.exitPointerLock();nearest=null;$('#interact').hidden=true;}
...
function action(type,{merchant,good,quantity,id}){
 ...
 if(type==='rest'&&merchant.role==='innkeeper'){result=rest(state);if(result.ok){controls.stamina=100;syncPlants();}}
...
function begin(){started=true;$('#menu').hidden=true;$('#hud').hidden=false;$('#marker').hidden=true;
 ...
 controls.yaw=state.position.yaw;controls.pitch=state.position.pitch;camera.position.set(state.position.x,surfaceAt(state.position.x,state.position.z)+1.72,state.position.z);syncPlants();resume();ui.update();
...
ui=createUI({state:()=>state,settings,started:()=>started,pause,resume,save,room:()=>world.houses.find(h=>h.id===currentRoom),action,title,
 reset(){state=freshState();controls.yaw=0;controls.pitch=0;for(const h of world.houses)if(h.open)world.toggleDoor(h);save();begin();},
 settingsChanged(){renderer.setPixelRatio(Math.min(devicePixelRatio,settings.quality==='high'?1.65:1));renderer.shadowMap.enabled=settings.quality==='high';audio.set();try{localStorage.setItem('mercantile.elderwood.settings',JSON.stringify(settings));}catch{}}
});
...
addEventListener('keydown',e=>{if(!started||e.repeat)return;if(e.code==='Escape'&&!ui.active&&!document.pointerLockElement){e.preventDefault();ui.open('settings');return;}if(paused||ui.active)return;if(e.code==='KeyE'){e.preventDefault();interact();}const keyPanel={KeyI:'inventory',KeyJ:'journal',KeyM:'map'}[e.code];if(keyPanel){e.preventDefault();ui.open(keyPanel);}});
...
function frame(now){requestAnimationFrame(frame);let dt=Math.max(0,Math.min(.05,(now-last)/1000));last=now;if(document.hidden)return;elapsed+=dt;
 if(started&&!paused){const m=controls.read(dt);moveWithCollision(state.position,m.dx,m.dz,world.colliders);state.position.yaw=controls.yaw;state.position.pitch=controls.pitch;
  if(m.moving)walkTime+=dt*(m.sprint?12:8);const bob=settings.reducedMotion?0:m.moving?Math.sin(walkTime)*.025:0;
  camera.position.set(state.position.x,T.MathUtils.damp(camera.position.y,surfaceAt(state.position.x,state.position.z)+1.72+bob,14,dt),state.position.z);camera.rotation.set(controls.pitch,controls.yaw,0,'YXZ');
  ...
  $('#stamina').style.width=controls.stamina+'%';audio.update(elapsed,m.moving,m.sprint,daylight);
 ...
 if(started&&elapsed-lastUI>.25){lastUI=elapsed;ui.update();}if(started&&!paused)updateMarker();else $('#marker').hidden=true;
 if(started&&!paused&&elapsed-lastSave>20){lastSave=elapsed;save();}
 renderer.render(scene,camera);performanceReport={frames:performanceReport.frames+1,drawCalls:renderer.info.render.calls,triangles:renderer.info.render.triangles};
}
requestAnimationFrame(frame);
addEventListener('resize',()=>{renderer.setSize(innerWidth,innerHeight);camera.aspect=innerWidth/innerHeight;camera.updateProjectionMatrix();});addEventListener('pagehide',()=>{if(started)save();});
