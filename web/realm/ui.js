...
export function createUI(api){let active=null,merchant=null,quantity=1,toastTimer;const dialog=$('#panel');
 const ui={
  ...
  close(shouldResume=true){if(!active)return;dialog.close();active=null;merchant=null;if(shouldResume&&api.started())api.resume();},
  render(){const s=api.state(),settings=api.settings;let title='',kicker='THE ELDERWOOD ROAD',html='';
   if(active==='trade'){
   ...
   $('#panel-title').textContent=title;$('#panel-kicker').textContent=kicker;$('#panel-body').innerHTML=html;if(active==='map')drawMap($('#large-map'),s);
  },
